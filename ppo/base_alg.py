...
class BasePolicyGradient:
    ...
    def _save_learning_curve(self, alg_name='PG'):
        """
            Save the learning curve data to a CSV file (only), for future plotting.
            
            Parameters:
                alg_name - algorithm name, used for file naming
        """
        import csv
        import os
        ...
        seed = self.seed if hasattr(self, 'seed') else 'unknown'

        if not self.logger.get('timesteps') or not self.logger.get('avg_ep_rews'):
            print(f"Warning: No data to save for {alg_name} on {env_name}")
            return

        timesteps = np.array(self.logger['timesteps'], dtype=np.float64)
        avg_rewards = np.array(self.logger['avg_ep_rews'], dtype=np.float64)

        filename = f'results/{alg_name}_{env_name}_seed{seed}.csv'

        with open(filename, mode='w', newline='') as file:
            writer = csv.writer(file)
            writer.writerow(['timestep', 'avg_ep_rew'])
            for t, r in zip(timesteps, avg_rewards):
                writer.writerow([t, r])
